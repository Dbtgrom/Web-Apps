...
elseif($file['size'] > 5000000){
 ...
 if($error == ''){
     $title = addslashes(mysqli_real_escape_string($link, $_POST['title-input']));
     $text = mysqli_real_escape_string($link,$_POST['text-input']);
     $image = $_FILES["image_input"];
     $image_name = basename($_FILES["image_input"]["name"]);
     $insert_post = enter_post($link,$_SESSION['id'], $image_name, $title, $text);
     $target_dir = "uploads/";
 ...
 <?php if(isset($posts)) :?>
        ...
        <div class="col-lg-3 col-md-6 mb-4">
          <div class="card">
              ...
              <img class="card-img-top" src="uploads/<?=$row['pic']?>"/>
            <div class="card-body">
              <h4 class="card-title"><?= htmlspecialchars($row['title'])?></h4>
              <p class="card-text"><?= htmlspecialchars($row['text'])?></p>
              <p class="card-text"><?= htmlspecialchars($row['createdTime'])?></p>
            </div>
            
              <?php if($_SESSION['id'] == $row['uid']) : ?>
            <div class="card-footer">
                <a href="edit.php?post_id=<?=$row['id']?>" class="btn btn-primary">Edit</a>
              <a href="delete.php?post_id=<?=$row['id']?>" class="btn btn-danger">Delete</a>
